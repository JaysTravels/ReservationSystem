...
namespace ReservationApi.Controllers
{
    ...
    [ApiController]
    public class FairPricePnrController : ControllerBase
    {
        private IPricePnrRepository _Repo;
        private ICacheService _cacheService;
        private readonly IMemoryCache _cache;
        public FairPricePnrController(IPricePnrRepository Repo, IMemoryCache memoryCache, ICacheService cacheService)
        {
            _Repo = Repo;
            _cache = memoryCache;
        ...
        //[Authorize]
        [HttpPost]
        public async Task<IActionResult> Post([FromBody] PricePnrRequest req)
        {

            ApiResponse res = new ApiResponse();

            var data = await _Repo.CreatePricePNRWithBookingClass(req);
